CREATE FUNCTION RML_TN_LC_14_NotaCreditoCliente
(
	IN id NVARCHAR(50),
...
BEGIN
	...
	THEN
		-- Indicador de la Nota de Credito
					...
					and T1."DocEntry" = :id)INTO R1 FROM DUMMY;
				
				--Tipo de Operacion	
					SELECT(select A."DocType" from ORIN A where A."DocEntry" =:id )INTO DOCTYPE FROM DUMMY;
			...
			THEN	
				   SELECT "CardCode","U_BPP_MDTD","U_BPP_MDSD","U_BPP_MDCD","CANCELED" INTO cc,tp,sr,sNumero,cancelado from ORIN WHERE "DocEntry"= TO_INTEGER(:id);

...
BEGIN 
  ...
  begin
	   ...
	   begin
		...
		--	 set @pepp = (SELECT count(*) FROM ODPO   
		...
		-------------------------------------------------------------------
		--	select VisOrder , * from DPO1
		
	 
			set @VisOrder=-777
		select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(OcrCode,'') = ''
		...
		if @VisOrder > 0 begin
			...
			set @VisOrder=-776
		select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(OcrCode2,'') = ''
		...
		if @VisOrder > 0 begin
			...
			set @VisOrder=-775
		select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(OcrCode3,'') = ''
		...
		if @VisOrder > 0 begin
			...
			set @VisOrder=-774
		select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(OcrCode4,'') = ''
		...
		select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(OcrCode,'') not in (
...
--				 
			...
			if @DataSource='I' begin
				...
				if @CANCELED='N' begin
					...
					--	if @SeriesName=(select U_STR_VALOR from [@STR_PARAM] where Code='SERMAN001') begin
							...
							--set @VisOrder=-1
							--select top 1 @VisOrder = VisOrder+1 from DPO1 i1 where i1.DocEntry = @DocEntry and ISNULL(Project,'') = ''
							...
							--from DPO1 i1 inner join OPRJ pr on pr.PrjCode=i1.Project and ISNULL(OcrCode,'') != ISNULL(pr.U_STR_CC2,OcrCode)
							...
							--set @VisOrder=-1
							--select top 1 @VisOrder = VisOrder+1 
							--from DPO1 i1 inner join OPRC cc on cc.PrcCode=i1.OcrCode2 and cc.U_STR_CLA9!=i1.OcrCode3
							...
							--set @VisOrder=-1
							--select Top 1 @VisOrder=VisOrder+1 from DPO1 P1 inner join OITM ar on ar.ItemCode=P1.ItemCode and ar.QryGroup3='N'
							...
							where t1.tot--Actual
									...
									ISNULL((select SUM(p1.LineTotal)
											from DPO1 p1
												inner join ODPO p0 on p0.DocEntry=p1.DocEntry
		...
		if ISNULL(@U_BPP_MDSD,'')!='' and ISNULL(@U_BPP_MDCD,'')!='' begin
			...
			update ODPO set NumAtCard = @NumAtCard,U_BPP_MDSD=@U_BPP_MDSD,U_BPP_MDCD=@U_BPP_MDCD where DocEntry = @DocEntry
			UPDATE JDT1 set  ref2=@NumAtCard where transid =(select top 1 transid from ODPO where DocEntry = @DocEntry )

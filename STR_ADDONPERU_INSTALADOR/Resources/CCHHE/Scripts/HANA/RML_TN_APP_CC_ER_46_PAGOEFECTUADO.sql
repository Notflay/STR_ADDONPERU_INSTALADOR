CREATE FUNCTION RML_TN_APP_CC_ER_46_PAGOEFECTUADO
(
	IN id NVARCHAR(50),
...
BEGIN
	...
	SELECT "U_BPP_TIPR" INTO tpornd FROM OVPM WHERE "DocEntry" = id;
		IF tpornd = 'CCH' OR tpornd = 'EAR'
			THEN
			--Validacion de seleccion de nro CCH - EAR * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
			SELECT COUNT('E') INTO cnt FROM OVPM WHERE RTRIM("U_BPP_NUMC") = '---' AND "DocEntry" =  id;
			IF cnt > 0
			THEN
			...
			SELECT CASE "U_BPP_TIPR" WHEN 'CCH' THEN (SELECT "U_BPP_ACCT" FROM "@BPP_CAJASCHICAS" WHERE "Code" = "U_BPP_CCHI") 
									 		...
									 		WHERE "DocEntry" = id;					
			SELECT "CashAcct" INTO ctapgo FROM OVPM	WHERE "DocEntry" = id;
			IF :ctargt != :ctapgo 
			THEN

...
IF Options <>'O2' THEN
	        ...
	        T0."TaxDate" AS "Fecha de Documento",
	         ...
	         -- Seleccionamos el "Cod_Bien" y "Cod_Operacion" de forma dinámica según el tipo de documento
	        (CASE T0."U_BPP_CtaTdoc" 
	    ...
	    WHERE T0."TransCode" = 'DTR'
	      ...
	      -- Filtros opcionales de proveedor y fechas
	      AND (
			    (IFNULL(:ProvD, '') = '' AND IFNULL(:ProvH, '') = '') -- No aplica filtro si ambos son vacíos
			    OR (IFNULL(:ProvH, '') = '' AND T1."ShortName" = :ProvD) -- Si ProvH es vacío, filtra solo por ProvD
			    OR T1."ShortName" BETWEEN :ProvD AND :ProvH -- Si ambos tienen valores, aplica el filtro de rango
			)
	     AND (
			    (IFNULL(:FcCntD, '') = '' AND IFNULL(:FcCntH, '') = '') -- No aplica filtro si ambas fechas están vacías
			    OR (IFNULL(:FcCntH, '') <> '' AND T0."RefDate" BETWEEN :FcCntD AND :FcCntH) -- Si ambas fechas tienen valor, aplica el filtro de rango
			    OR (IFNULL(:FcCntH, '') = '' AND T0."RefDate" >= CAST(CAST(:FcCntD AS DATE) AS nvarchar(10))) -- Si FcCntH es vacío, filtra solo por FcCntD
			)
	      AND (
			    (IFNULL(:FcVncD, '') = '' AND IFNULL(:FcVncH, '') = '') -- No aplica filtro si ambas fechas están vacías
			    OR (IFNULL(:FcVncH, '') <> '' AND T0."DueDate" BETWEEN :FcVncD AND :FcVncH) -- Si ambas fechas tienen valor, aplica el filtro de rango
			    OR (IFNULL(:FcVncH, '') = '' AND T0."DueDate" >= CAST(CAST(:FcVncD AS DATE) AS nvarchar(10))) -- Si FcCntH es vacío, filtra solo por 
			)
	...
	ELSE
		    ...
		    CASE T0."ObjType"
		        ...
		        WHEN '19' THEN (SELECT "U_BPP_CdOp" FROM ORPC WHERE "DocEntry" = T0."DocEntry")
		    END AS "Cod_Operacion",
		    (CONCAT('Detracción ', ROUND(T0."U_STR_TasaDTR", 2) || ' %')) AS "Detraccion",
		    '' AS "Cod_Detraccion",
			...
			WHERE 
			    T0."U_STR_TasaDTR" > 0 
			    AND (IFNULL(T0."U_STR_DetraccionPago", '') = '' OR T0."U_STR_DetraccionPago" = 'N')
			    AND T0."CANCELED" = 'N' 
			    ...
			    AND (
				    (IFNULL(:ProvD, '') = '' AND IFNULL(:ProvH, '') = '') -- No aplica filtro si ambos son vacíos
				    OR (IFNULL(:ProvH, '') = '' AND T0."CardCode" = :ProvD) -- Si `ProvH` está vacío, filtra solo por `ProvD`
				    OR T0."CardCode" BETWEEN :ProvD AND :ProvH -- Si ambos tienen valor, aplica el filtro de rango
				)
			    
			    -- Condiciones de Fecha de Contabilización
			    AND (
				    (IFNULL(:FcCntD, '') = '' AND IFNULL(:FcCntH, '') = '') -- No aplica filtro si ambas fechas están vacías
				    OR (IFNULL(:FcCntH, '') <> '' AND T0."DocDate" BETWEEN :FcCntD AND :FcCntH) -- Si ambas fechas tienen valor, aplica el filtro de rango
				    OR (IFNULL(:FcCntH, '') = '' AND T0."DocDate" >= CAST(CAST(:FcCntD AS DATE) AS nvarchar(10))) -- Si `FcCntH` está vacío, filtra solo por `FcCntD`
				)
			    
			    -- Condiciones de Fecha de Vencimiento
			    AND  (
					(IFNULL(:FcVncD, '') = '' AND IFNULL(:FcVncH, '') = '') -- No aplica filtro si ambas fechas están vacías
					    OR (IFNULL(:FcVncH, '') <> '' AND T0."DocDueDate" BETWEEN :FcVncD AND :FcVncH) -- Si ambas fechas tienen valor, aplica el filtro de rango
					    OR (IFNULL(:FcVncH, '') = '' AND T0."DocDueDate" >= CAST(CAST(:FcVncD AS DATE) AS nvarchar(10))) -- Si `FcVncH` está vacío, filtra solo por `FcVncD`
					)

...
AS
BEGIN
    DECLARE @vv_sql NVARCHAR(MAX);
    DECLARE @vv_condicion NVARCHAR(100);

    SET @vv_condicion = '';

    SELECT @vv_condicion = 
        CASE 
            WHEN @pv_Moneda <> '' THEN ' AND T1."U_LET_MON" = ''' + @pv_Moneda + '''' 
            ELSE '' 
        END
        + 
        CASE 
            WHEN @pv_ShortName <> '' THEN ' AND T0."ShortName" = ''' + @pv_ShortName + '''' 
            ELSE '' 
        END;
        ...
        FROM 
            (    
                SELECT 
                    ...
                    ,ISNULL(T3."U_MntPrc",T4."U_MntPrc") AS "Percepcion"
                    ,T1."RefDate" AS "FecAce"
                    ,"FcTotal"
                    ,"LocTotal"
                    ,"BalDueDeb"
                    ,"BalDueCred"
                    ,REPLICATE('' '', 10) AS "NumUnico"
                ...
                WHERE 
                    ...
                    AND LEFT(T1."Ref2",3) = ''LET'' 
                    AND T1."U_LET_EST" = ''' + @pv_Cartera + ''' 
                    ' + @vv_condicion + '
            ) T  
        ORDER BY ' + @pv_IndiceColumna + ' ' + @pv_Ordenamiento;

    EXEC sp_executesql @vv_sql;
END;

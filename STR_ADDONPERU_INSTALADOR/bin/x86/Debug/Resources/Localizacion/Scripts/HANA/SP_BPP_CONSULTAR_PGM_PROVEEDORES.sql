...
BEGIN
	...
	SELECT 
		...
		ELSE
			CASE WHEN T3."DocCur" = 'SOL' THEN  T3."DocTotal" - T3."PaidSum" -  IFNULL(T3."WTSum",0) ELSE T3."DocTotalFC" - T3."PaidSumFc" -  IFNULL(T3."WTSumFC",0) END 
		END "MontoPago"
		,CASE WHEN T3."Installmnt" > 1 THEN
			CASE WHEN T3."DocCur" = 'SOL' THEN  T7."InsTotal" -   T7."PaidToDate" -  IFNULL(T7."WTSum",0) ELSE T7."InsTotalFC" - T7."PaidFC" -  IFNULL(T7."WTSumFC",0) END 
		ELSE
		...
		,
		CASE WHEN T3."Installmnt" > 1 THEN
			CASE WHEN T3."DocCur" = 'SOL' THEN  IFNULL(T7."WTSum",0) ELSE  IFNULL(T7."WTSumFC",0) END 
		ELSE
			CASE WHEN T3."DocCur" = 'SOL' THEN  IFNULL(T3."WTSum",0) ELSE  IFNULL(T3."WTSumFC",0) END 
		END
		"ImporteRetencion",
		CASE WHEN IFNULL( T8."AbsEntry",0) = 0 THEN 'No Aplica' ELSE 'Si aplica' END "DetraccionPago"
	FROM OPCH T3 
	...
	WHERE 
		...
		AND IFNULL(T6."U_BPP_NUMSAP",0)= 0 )TT1
	 WHERE TT1."Saldo" >0
	...
	SELECT 
		...
		,CASE WHEN T3."DocCur" = 'SOL' THEN  T3."DocTotal" - T3."PaidSum"  -  IFNULL(T3."WTSum",0) ELSE T3."DocTotalFC" - T3."PaidSumFc" -  IFNULL(T3."WTSumFC",0) END "MontoPago"
		,CASE WHEN T3."DocCur" = 'SOL' THEN  T3."DocTotal" - T3."PaidSum" -  IFNULL(T3."WTSum",0) ELSE T3."DocTotalFC" -T3."PaidSumFc" -  IFNULL(T3."WTSumFC",0)   END "Saldo"
		,IFNULL(T3."DocCur",'') "Moneda"
		,'Factura' "TipoDocumento"
		,T3."CardCode" "CodigoProveedor"
		,T3."CardName" "NombreProveedor"
		...
		,T7."InstlmntID" "NumeroCuota"
		,CAST(T7."InstlmntID" AS NVARCHAR(10)) || 'de' || CAST(T3."Installmnt"AS NVARCHAR(10))  "NombreCuota"
		,CASE WHEN T3."DocCur" = 'SOL' THEN  IFNULL(T3."WTSum",0) ELSE  IFNULL(T3."WTSumFC",0) END "ImporteRetencion",
		CASE WHEN IFNULL( T8."AbsEntry",0) = 0 THEN 'No Aplica' ELSE 'Si aplica' END "DetraccionPago"
	FROM ODPO T3 
	INNER JOIN OCRD T4 ON T3."CardCode" = T4."CardCode"
	LEFT JOIN OCRB T5 ON T4."BankCode" = T5."BankCode" AND T4."CardCode" = T5."CardCode"
